...
def configure():
    ...
    return {
        ...
        "API_KEY":"3d0905b31ca1f9ef7339ee2a2490066b",
...
def fetch_fixture_lineups(api_host, api_key, fixture_id):
    try:
        ...
        if 'errors' in data_dict and isinstance(data_dict['errors'], dict) and data_dict['errors'].get('requests'):
            msg = f"🚫 Limite diário atingido ao consultar fixture {fixture_id}: {res.status} - {data_dict}."
            logging.error(msg)
            print(msg)
            return None, "limit"

        if 'response' in data_dict and not data_dict['response']:
            msg = f"⚠️ API retornou resposta vazia para fixture {fixture_id}: {res.status} - {data_dict}."
            logging.warning(msg)
            print(msg)
            return None, "empty"
        ...
        msg = f"⚠️ Resposta inesperada para fixture {fixture_id}: {res.status} - {data_dict}."
        logging.warning(msg)
        print(msg)
        return None, "unknown"

    except Exception as e:
        msg = f"❌ Erro ao buscar fixture {fixture_id}: {res.status} - {data_dict} - {type(e).__name__}: {str(e)}"
        logging.error(msg)
        print(msg)
        return None, "exception"

#%%
def transform_lineup(df_lineup, df_all_lineups):
    df_all_lineups = df_all_lineups.drop_duplicates()
    all_rows = []
    ...
    all_rows = pd.concat([df_all_lineups, pd.DataFrame(all_rows)], ignore_index=True)
    all_rows.drop_duplicates(subset="fixture.id", keep="last", inplace=True)
    return pd.DataFrame(all_rows)

#%%
def filter_fixtures(df_fixture_load, df_status):
    ignore_ids = df_status[df_status['status'].isin(['success', 'empty_response'])]['fixture.id'].astype(str)
    ...
    for i in range(0, len(df_fixture_load), config['BATCH_SIZE']):
        ...
        for fixture_id in batch['fixture.id']:
            ...
            if status == "success":
                ...
                print(f"🚫 Limite da API atingido ao consultar fixture {fixture_id}")
                limit_reached = True
                print("⚠️ Limite diário atingido. Interrompendo todas as consultas.")
                break

            elif status == "empty":
                print(f"⚠️ Fixture {fixture_id} retornou vazio.")
                logging.warning(f"⚠️ Fixture {fixture_id} retornou vazio.")

            else:
                print(f"❌ Erro na fixture {fixture_id}. Status: {status}")
                logging.error(f"❌ Erro na fixture {fixture_id}. Status: {status}")
...
#%%
lineup_files = glob(os.path.join(BASE_DIR, "fixture_lineup/*fixture_lineup.csv"))
if lineup_files:
    # Ler e concatenar todos os arquivos
    df_all_lineups = pd.concat([pd.read_csv(f) for f in lineup_files], ignore_index=True)
    df_all_lineups = df_all_lineups.drop_duplicates()
    display(df_all_lineups)
else:
    print("⚠️ Nenhum arquivo 'fixture_lineup.csv' encontrado no diretório.")
#%%
...
#%%
df_lineup_flat = transform_lineup(fixture_lineups, df_all_lineups) if not fixture_lineups.empty else pd.DataFrame()
#(df, filepath)
save_if_not_empty(df_final_status, "status", f"fixture_status_lineup.csv")
save_if_not_empty(df_lineup_flat, "fixture_lineup", f"{timestamp}_fixture_lineup.csv")
save_if_not_empty(df_all_lineups, "fixture_lineup/compilado/", f"fixture_lineup.csv")
