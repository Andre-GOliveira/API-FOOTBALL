...
consulted_fixtures_list = load_from_csv(os.path.join(BASE_DIR, "consulted/consulted_fixtures_lineup.csv"))
df_errors = load_from_csv(os.path.join(BASE_DIR, "errors/error_fixtures_lineup_vazios.csv"))
...
df_Fixtures_fc = df_fixture_load[FixuresColumns]
df_fixture_load = df_fixture_load[
    ~df_fixture_load['fixture.id'].astype(str).isin(consulted_fixtures_list['fixture.id'].astype(str))] 
    #& ~df_Fixtures_fc['fixture.id'].astype(str).isin(df_errors['fixture.id'].astype(str))]

df_fixture_load.reset_index(drop=True, inplace=True)
display(df_fixture_load)

#%%
FixuresColumns = ['fixture.id']
df_fixture_load = df_fixture_load[FixuresColumns]
display(df_fixture_load)
...
limit_reached = False  # controla se o limite foi atingido
#%%
for i in range(0, len(df_fixture_load), batch_size):
    if limit_reached:
    # caso queira checar antes de entrar no loop 
    # do batch, apenas para segurança.
        break
...
# %%
save_if_not_empty(pd.DataFrame({'fixture.id': consulted_fixtures_list}), "consulted", f"consulted_fixtures_lineup.csv")
save_if_not_empty(pd.DataFrame({'fixture.id': error_fixtures}), "errors", f"error_fixtures_lineup.csv")
save_if_not_empty(pd.DataFrame({'fixture.id': limit_error_fixtures}), "errors", f"limit_error_fixtures_lineup.csv")
save_if_not_empty(pd.DataFrame({'fixture.id': empty_response_fixtures}), "errors", f"empty_response_fixtures_lineup.csv")
